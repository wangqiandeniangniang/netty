...
public abstract class Serializer {
	...
	public static final Charset CHARSET = Charset.forName("UTF-8");//设置字符串编码
	...
	public Serializer readFromBytes(byte[] bytes) {
		readBuffer = BufferFactory.getBuffer(bytes); //从数组获取buffer
		read(); //从buffer中读取数据
		readBuffer.clear(); //清空buffer 数据
		return this; //返回本对象
	}
	...
	public void readFromBuffer(ChannelBuffer readBuffer) {
		this.readBuffer = readBuffer; //直接读取buffer
		read();
	}
	...
	public ChannelBuffer writeToLocalBuff(){
		writeBuffer = BufferFactory.getBuffer(); //获取buffer 然后写入
		write(); //写入属性
		return writeBuffer; //返回该buffer
	}
	...
	public ChannelBuffer writeToTargetBuff(ChannelBuffer buffer){
		writeBuffer = buffer; //写入目标buffer，也就是将数据写入该buffer 中， 写入自己类的buffer
		write();
		return writeBuffer;
	}
	...
	public byte[] getBytes() {
		writeToLocalBuff();  //先写入buffer,然后从buffer读取到字节数组
		byte[] bytes = null;
	...
	public <I> I read(Class<I> clz) {
		Object t = null;
		if ( clz == int.class || clz == Integer.class) { // int.class Integer.class
			t = this.readInt();
			...
			t = readString();
		} else if (Serializer.class.isAssignableFrom(clz)){ //是序列对象
			try {
				byte hasObject = this.readBuffer.readByte(); //写对象
				if(hasObject == 1){
